...
mod tests {
	...
	// Tests for 16-byte block size (AES)
	#[test]
	fn pad_16_byte_block() {
		let padded = PKCS7PaddedData::<16>::pad(&[1, 2, 3, 4, 5]);
		let blocks: Vec<_> = padded.into_iter().collect();
		assert_eq!(blocks.len(), 1);
		assert_eq!(
			blocks[0],
			[1, 2, 3, 4, 5, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11, 11]
		);
	}

	#[test]
	fn roundtrip_16_byte_blocks() {
		for len in 0..=48 {
			let input: Vec<u8> = (0..len).map(|i| i as u8).collect();
			let padded = PKCS7PaddedData::<16>::pad(&input);
			let blocks: Vec<_> = padded.into_iter().collect();

			let reparsed = PKCS7PaddedData::<16>::try_from_blocks(blocks).unwrap();
			assert_eq!(reparsed.into_unpadded(), input, "failed for len={}", len);
		}
	}
}
